...
export const Tv = () => {
    ...
    const {data, isLoading, isError} = useQuery(['movie', currentT], () => FilmService.getFilms(movieTop), {
                ...
                <div className="container">
                    <div className='mb-[80px]'>
                        ...
                        <div className="flex items-center justify-center gap-[30px] text-[20px]">
                                <button onClick={() => setCurrentT(currentT - 1)} disabled={currentT === 1} className={`bg-[#EBEEF5] p-[10px] rounded-[12px] rotate-[180deg] transition-all ease-in-out duration-[.3s] hover:bg-[#b8b8b8] ${currentT > 1 ? 'block' : 'hidden'}`}><GrNext /></button>
                                {data?.items?.length > 0 &&  
                                     <button onClick={() => setCurrentT(currentT + 1)} disabled={currentT >= 35} className={`bg-[#EBEEF5] p-[10px] rounded-[12px] transition-all ease-in-out duration-[.3s] hover:bg-[#b8b8b8] ${currentT < 35 ? 'block' : 'hidden'}`}><GrNext /></button>
                                }
                        </div>   
                    </div>
                </div>
